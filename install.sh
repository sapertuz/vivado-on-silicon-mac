...
/usr/local/bin/docker run -it --init --rm -v /tmp/.X11-unix:/tmp/.X11-unix -v $script_dir:/home/user --platform linux/amd64 x64-linux bash /home/user/docker.sh
...
echo "/usr/local/bin/docker run --init --rm --name vivado_container -v /tmp/.X11-unix:tmp/.X11-unix -v $script_dir:home/user --platform linux/amd64 x64-linux sudo -H -u user bash /home/user/start_vivado.sh &" >> Launch_Vivado.app/Launch_Vivado
